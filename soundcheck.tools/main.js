const TOOLS = [
  ...
  { id: 'portalsT',  name: 'Package Tracking Portal',  icon: '🫆', url: 'https://www.trackinthecloud.com/mysoundchecknashville/Signin.aspx?ReturnUrl=/mysoundchecknashville/default.aspx', accessIndex: 3 },
  { id: 'portalsN',  name: 'Tracking Number Search',  icon: '🔎', url: 'https://www.trackinthecloud.com/soundchecknashville/', accessIndex: 3 },
...
function groupByAccessIndex(tools) {
  const groups = [];
  for (const t of tools) {
    const last = groups[groups.length - 1];
    if (last && last[0].accessIndex === t.accessIndex) last.push(t);
    else groups.push([t]);
  }
  return groups;
}
...
function render(isAdmin, access) {
  ...
  const groups = groupByAccessIndex(visible);
