...
class Reservation(db.Model):
    ...
    SENTINEL_YEAR = 9999   # end_date.year for an indefinite/utility booking

    @staticmethod
    def _is_day_level(all_day, start_time, end_time, end_date):
        """True when a booking occupies whole calendar days rather than a timed
        slot — i.e. all-day, indefinite/utility, or a 00:00-23:59 span. For these
        the 30-min turnaround buffer (a same-day truck-spacing concern) must NOT
        apply, so it can't bleed across midnight into a neighbouring day."""
        from datetime import time as _t
        return bool(all_day) or end_date.year == Reservation.SENTINEL_YEAR \
            or (start_time == _t(0, 0) and end_time == _t(23, 59))

    @staticmethod
    def has_conflict(dock_numbers, start_date, end_date, start_time, end_time,
                     leeway_minutes=0, exclude_id=None, all_day=False):
        ...
        new_day_level = Reservation._is_day_level(all_day, start_time, end_time, end_date)
        ...
        for r in query.all():
            if not dock_set.intersection(set(r.dock_numbers)):
                continue
            existing_day_level = Reservation._is_day_level(
                r.all_day, r.start_time, r.end_time, r.end_date)

            # If either booking owns whole days, conflict is a pure date-range
            # question (inclusive). An indefinite booking starting tomorrow does
            # not block a booking today, and consecutive-day bookings don't need a
            # cross-midnight buffer. This also sidesteps the year-9999 datetime
            # overflow, since the sentinel only ever appears on day-level bookings.
            if new_day_level or existing_day_level:
                if start_date <= r.end_date and end_date >= r.start_date:
                    return True
                continue

            # Both are timed slots on the same dock: apply the buffer in real time.
